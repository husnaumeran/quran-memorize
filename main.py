...
def chapter_select(chapters): return Select(*[Option(f"{c['id']}. {c['name_simple']}", value=c['id']) for c in chapters], id="chapter", name="chapter", cls="w-full p-2 border rounded")

@app.get("/", response_class=HTMLResponse)
def home():
    chapters = get_chapters()
    return Html(
        Head(Title("Quran Memorize"),
        ...
        Body(
            Div(
                ...
                Div(
                    Form(
                        Div(
                            Div(Label("Chapter", cls="block mb-1"), chapter_select(chapters), cls="mb-4"),
                            Div(Label("Start Verse", cls="block mb-1"), Input(type="number", name="start", value="1", min="1", cls="w-full p-2 border rounded"), cls="mb-4"),
                            Div(Label("End Verse", cls="block mb-1"), Input(type="number", name="end", value="5", min="1", cls="w-full p-2 border rounded"), cls="mb-4"),
                            Div(Label("Repeats", cls="block mb-1"), Input(type="number", name="repeats", value="3", min="1", max="10", cls="w-full p-2 border rounded"), cls="mb-4"),
                            cls="grid grid-cols-2 md:grid-cols-4 gap-4"
                        ),
                        Button("Start Memorizing", type="submit", cls="mt-4 w-full bg-blue-600 text-white py-3 rounded-lg text-lg hover:bg-blue-700"),
                        hx_post="/memorize", hx_target="#session", hx_swap="innerHTML"
                    ),
                    cls="bg-white p-6 rounded-lg shadow mb-6"
                ),
                Div(id="session"),
                cls="max-w-4xl mx-auto p-4"
            ),
